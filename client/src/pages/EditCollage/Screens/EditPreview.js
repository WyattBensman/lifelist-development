...
import Participants from "../../CreateCollage/Popups/Participants";
import HeaderStack from "../../../components/Headers/HeaderStack";
import Icon from "../../../components/Icons/Icon";
import { UPDATE_COLLAGE } from "../../../utils/mutations";
...
export default function EditPreview() {
  ...
  const { collage, updateCollage } = useCreateCollageContext(); // Access collage data from context
  ...
  const [updateCollageMutation] = useMutation(UPDATE_COLLAGE, {
    onCompleted: () => {
  ...
  const handleUpdateCollage = () => {
    ...
    if (!images || images.length === 0) {
      alert("Please select at least one image to update the collage.");
      return;
    }
    ...
    updateCollageMutation({
      variables: {
        collageId: collage._id, // Assuming collage has an `_id` property
        ...
        coverImage: coverImage || imagePaths[0], // Use coverImage or default to the first image
      },
    })
      .then(() => {
        console.log("Collage updated successfully!");
        navigation.navigate("MainFeedHome", { refresh: true });
      })
      .catch((error) => {
  ...
  return (
    <View style={layoutStyles.wrapper}>
      ...
      <HeaderStack
        ...
        button1={
          <Pressable onPress={handleUpdateCollage}>
            <Text style={styles.createButtonTextActive}>Update</Text>
          </Pressable>
        }
      />
