...
const FilteredCamera = ({ filter, facing, flash, zoom }) => {
  const shader = shaders[filter] || null;

  return (
    <View style={{ flex: 1 }}>
      <Camera style={{ flex: 1 }} type={facing} flashMode={flash} zoom={zoom}>
        <Surface style={{ flex: 1 }}>
          <Node shader={shader} uniforms={{ image: Camera }} />
        </Surface>
      </Camera>
    </View>
  );
